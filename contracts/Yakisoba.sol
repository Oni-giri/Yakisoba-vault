...
/// @title Yakisoba vault token
/** @notice This contract is an ERC4626 cross-chain vault that allows users to deposit assets
 *  and mint Yakisoba tokens. The Yakisoba tokens can be used to redeem the assets on the same chain.
 ...
 **/
contract Yakisoba is Pausable, ReentrancyGuard, Ownable, ERC20Snapshot {
  ...
  //////////////////////////////////////////////////////////////*/
	...
	error ExtraFunds(uint256 extraFunds);
  ...
  //////////////////////////////////////////////////////////////*/
	...
	event Deposit(
		...
		address indexed owner, // who received the yakisoba tokens
		uint256 assets, // ex: amount of USDC sent
		uint256 shares // amount of yakisoba tokens minted
	);
	event Withdraw(
  ...
  //////////////////////////////////////////////////////////////*/
	...
	//TODO: remove this, unused param
	mapping(uint256 => address) public bridgeMap; // BridgeConnectors that can be used
  ...
  //////////////////////////////////////////////////////////////*/
	...
	function _deposit(
		...
		if (_receiver == address(this)) revert YakisobaCantBeReceiver();
	...
	function dispatchAssets(
		...
		// We check if we have extra funds
		if (budget > 0) revert ExtraFunds(budget);
	}
	...
	function increaseLiquidity(uint256 _liquidityAdded) external onlyOwner {
	...
	/// @dev We consider each chain/pool as having "debt" to the yakisoba
	...
	/// @notice Decimals of the yakisoba token
	/// @return The number of decimals of the yakisoba token
	function decimals() public view override returns (uint8) {
		return (tokenDecimals);
	}

	/// @notice The share price equal the amount of assets redeemable for one yakisoba token
	/// @return The virtual price of the yakisoba token
	...
	/// @notice Convert how much yakisoba tokens you can get for your assets
	/// @param _assets Amount of assets to convert
	/// @return The amount of yakisoba tokens you can get for your assets
	...
	/// @notice Convert how much asset tokens you can get for your yakisoba tokens
	/// @dev Bear in mind that some negative slippage may happen
	/// @param _shares amount of shares to covert
	/// @return The amount of asset tokens you can get for your yakisoba tokens
	...
	function setMaxTotalAssets(uint256 _amount) external onlyOwner {
	...
	/// @notice Unpause the yakisoba
	/// @dev We seed the yakisoba with 1e8 tokens if it's empty
	function unpause() external onlyOwner {
		_unpause();
	}

	/// @notice Take a snapshot of the yakisoba balances
	function snapshot() external onlyOwner returns (uint256 id) {
		id = _snapshot();
		emit Snapshot(id);
	}
	...
	function rescueToken(address _token, bool _onlyETH) external onlyOwner {
