...
// TODO: migrate requires to errors
 ...
 * deployment size.
 */
contract MockSwapReturnsNothing is Ownable, ReentrancyGuard {
		...
		returns (uint256)
	{
		...
		if (
			swapStorage.pooledTokens[REAL_ASSET_INDEX].balanceOf(
				address(this)
			) < swapStorage.balances[REAL_ASSET_INDEX]
		) {
			revert WrongBalance(
				swapStorage.pooledTokens[REAL_ASSET_INDEX].balanceOf(
					address(this)
